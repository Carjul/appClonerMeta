...
# CONFIG
# ============================================================
# ⚠️ SECURITY: /metricas NO usa tokens desde .env.
# Los tokens se leen desde MongoDB: meta_automation.configs.access_token.
def _setting(name: str, default: str = "") -> str:
    """Read config from env, root core.settings, or backend app.config."""
    value = os.environ.get(name)
    if value:
        return value
    try:
        from backend.core.config import settings as core_settings
        value = getattr(core_settings, name, None)
...
MONGO_URI = _setting("MONGO_URI", os.environ.get("MONGODB_URI", "mongodb://localhost:27017"))
MONGO_DB_NAME = _setting("DB_NAME", os.environ.get("MONGODB_DB", "meta_automation"))
CONFIGS_COLLECTION = os.environ.get("METRICS_CONFIGS_COLLECTION", "configs")
TOKEN_CACHE_TTL = int(os.environ.get("METRICS_TOKEN_CACHE_TTL", "60"))

API_VERSION = "v21.0"
BASE = f"https://graph.facebook.com/{API_VERSION}/"

_token_cache_lock = threading.Lock()
_token_cache: dict[str, Any] = {"tokens": {}, "configs": [], "loaded_at": 0.0}

INSIGHTS_FIELDS = "spend,impressions,cpm,cpc,ctr,unique_link_clicks_ctr,actions,cost_per_action_type,purchase_roas"
METRICS_DATA_DIR = Path(os.environ.get("METRICS_DATA_DIR", Path(__file__).resolve().parent / "data" / "metrics"))
METRICS_DATA_DIR.mkdir(parents=True, exist_ok=True)
CAMPAIGN_CACHE_FILE = str(METRICS_DATA_DIR / "campaigns_cache.json")
...
def _is_valid_token(token: str) -> bool:
    ...
    return bool(token) and token not in ("YOUR_BM1_TOKEN_HERE", "YOUR_BM2_TOKEN_HERE", "PLACEHOLDER")


def _load_tokens_from_mongo(force_refresh: bool = False) -> dict[str, str]:
    """Load the first two configured tokens from MongoDB configs collection.

    This mirrors the existing frontend behavior: /api/configs is sorted by
    created_at desc and the dashboard takes the first two configs as BM1/BM2.
    Tokens are cached briefly to avoid hitting MongoDB on every API call.
    """
    now = _time.time()
    with _token_cache_lock:
        cached = dict(_token_cache.get("tokens") or {})
        loaded_at = float(_token_cache.get("loaded_at") or 0.0)
        if not force_refresh and cached and now - loaded_at < TOKEN_CACHE_TTL:
            return cached

    tokens: dict[str, str] = {}
    configs_meta: list[dict[str, Any]] = []
    try:
        client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=3000)
        col = client[MONGO_DB_NAME][CONFIGS_COLLECTION]
        docs = list(
            col.find({"access_token": {"$exists": True, "$ne": ""}}, {"name": 1, "bm_id": 1, "access_token": 1, "created_at": 1})
            .sort("created_at", -1)
            .limit(2)
        )
        for idx, doc in enumerate(docs, start=1):
            ...
            configs_meta.append({
                "bm": bm,
                "name": doc.get("name") or bm,
                "bm_id": doc.get("bm_id") or "",
                "_id": str(doc.get("_id")),
            })
    except Exception as exc:
        print(f"[metricas] ERROR loading Mongo tokens: {exc}", flush=True)

    with _token_cache_lock:
        _token_cache["tokens"] = tokens
        _token_cache["configs"] = configs_meta
        _token_cache["loaded_at"] = now
    return tokens


def get_token(bm):
    """Get access token for Business Manager from MongoDB configs."""
    return _load_tokens_from_mongo().get(bm, "")


def has_token(bm):
    """Check if Business Manager has a valid MongoDB token."""
    return bool(get_token(bm))


def live_mode() -> bool:
    """True when at least one token exists in MongoDB configs."""
    return bool(_load_tokens_from_mongo())
...
def fetch_all_live(date_preset, force_refresh_accounts=False):
    # Fast path for dashboard load: use the two MongoDB tokens to discover
    # accounts, then fetch account-level insights only. Campaign drill-down is
    # loaded lazily when the user clicks an account, avoiding long bootstraps.
    accounts_with_token = get_accounts(force_refresh=force_refresh_accounts)

    results = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
        futures = {ex.submit(fetch_account, a, date_preset): a for a in accounts_with_token}
        for f in concurrent.futures.as_completed(futures):
            row = f.result()
            if row.get("spend", 0) > 0 or row.get("purchases", 0) > 0:
                results.append(row)
...
# FEEDBACK MANAGEMENT
# ============================================================
FEEDBACK_FILE = str(METRICS_DATA_DIR / "feedback.json")
...
def api_token_status():
    tokens = _load_tokens_from_mongo(force_refresh=True)
    with _token_cache_lock:
        configs = list(_token_cache.get("configs") or [])
    return jsonify({
        "source": "mongo",
        "database": MONGO_DB_NAME,
        "collection": CONFIGS_COLLECTION,
        "tokenCount": len(tokens),
        "configured": sorted(tokens.keys()),
        "configs": configs,
    })
...
.auth-chip{font-size:12px;color:var(--text2);border:1px solid var(--border);border-radius:999px;padding:5px 10px;background:var(--bg)}
.logout-link{font-size:12px;color:#fff;background:#ef4444;border-radius:8px;padding:7px 10px;text-decoration:none;font-weight:700}
...
<div class="header">
  ...
  <div class="hdr-r">
    ...
    <span class="auth-chip" id="authChip">Usuario</span>
...
function toggleCamps(acctId, bm){
  ...
  renderTable();
  if(!campCache[acctId]){
    fetch(`/metricas/api/campaigns?account_id=${acctId}&bm=${bm}&period=${currentPeriod}&refresh=1`)
      .then(r=>r.json())
...
function loadCreativeLinks(){
  fetch("/metricas/api/creative-links")
    .then(r=>r.json())
    .then(data=>{
...
function loadAuthUser(){
  fetch("/api/auth/me", {headers:{"Accept":"application/json"}})
    ...
    .then(d=>{
      const u=d&&d.user;
      if(u) $("#authChip").textContent=`${u.name} · ${u.role}`;
    })
    .catch(()=>{});
}

function checkAutoReport(){
  fetch("/metricas/api/report").then(r=>r.json()).then(d=>{
...
function doLoad(period,force=false){
  ...
  fetch(`/metricas/api/bootstrap${force?"?refresh=1":""}`)
    .then(r=>r.json())
    .then(data=>{
      periodRowsCache={
...
function submitFeedback(){
  ...
  const endpoint=currentEditingFbId?`/metricas/api/feedback/${currentEditingFbId}`:"/metricas/api/feedback";
...
function loadFeedbacks(){
  fetch("/metricas/api/feedback").then(r=>r.json()).then(data=>{feedbacks=data||[];renderFeedbacks();}).catch(()=>{});
}
...
function submitResponse(){
  ...
  fetch(`/metricas/api/feedback/${currentResponseFbId}/response`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:text})})
    .then(r=>r.json())
...
function toggleStatus(fbId,newStatus){
  fetch(`/metricas/api/feedback/${fbId}/status`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({status:newStatus})})
    .then(r=>r.json())
    .then(()=>{loadFeedbacks();})
    .catch(e=>console.error(e));
}
...
function deleteFeedback(fbId){
  ...
  fetch(`/metricas/api/feedback/${fbId}/delete`,{method:"DELETE"})
    .then(r=>r.json())
    .then(()=>{loadFeedbacks();})
    .catch(e=>console.error(e));
}
