...
def create_meta_connection(
    ...
    return RedirectResponse("/dashboard/setup?connection_saved=1", status_code=303)
...
def activate_meta_connection(connection_id: int, db: MongoSession = Depends(get_db)):
    ...
    return RedirectResponse("/dashboard/setup?connection_active=1", status_code=303)
        ...
        if connection.business_id:
            for business in result["businesses"]:
                if business.get("id") == connection.business_id:
                    ...
                    break
        db.commit()
        return RedirectResponse("/dashboard/setup?token_ok=1", status_code=303)
    except Exception as exc:
        connection.is_valid = False
        connection.last_error = str(exc)
        connection.last_tested_at = datetime.utcnow()
        db.commit()
        return RedirectResponse("/dashboard/setup?token_ok=0", status_code=303)
...
def delete_meta_connection(connection_id: int, db: MongoSession = Depends(get_db)):
    ...
    return RedirectResponse("/dashboard/setup?connection_deleted=1", status_code=303)
...
def setup_save(
    ...
    db.commit()
    return RedirectResponse("/dashboard/setup?saved=1", status_code=303)
...
def test_notification():
    ...
    return RedirectResponse(f"/dashboard/setup?test_telegram={int(res['telegram'])}&test_slack={int(res['slack'])}", status_code=303)
