...
async def create_set(cat_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if not selected_ids:
        return RedirectResponse(f"/dashboard/catalogs/{cat_id}/sets/new?error=select_at_least_one", status_code=303)
    ...
    if sync_to_meta and not cat.fb_catalog_id.startswith("local-"):
        try:
            ...
            return RedirectResponse(f"/dashboard/catalogs/{cat_id}/sets/new?error={str(e)[:80]}", status_code=303)

    pset = ProductSet(catalog_id=cat_id, name=name, retailer_ids=retailer_ids, fb_set_id=fb_set_id)
    db.add(pset)
    db.commit()

    return RedirectResponse(f"/dashboard/catalogs/{cat_id}/sets", status_code=303)
...
async def update_set(cat_id: int, set_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if not selected_ids:
        return RedirectResponse(f"/dashboard/catalogs/{cat_id}/sets/{set_id}/edit?error=select_at_least_one", status_code=303)
    ...
    if sync_to_meta and not cat.fb_catalog_id.startswith("local-"):
        try:
            ...
            return RedirectResponse(f"/dashboard/catalogs/{cat_id}/sets/{set_id}/edit?error={str(e)[:80]}", status_code=303)
...
def sync_set(cat_id: int, set_id: int, db: Session = Depends(get_db)):
    ...
    if cat.fb_catalog_id.startswith("local-"):
        return RedirectResponse(f"/dashboard/catalogs/{cat_id}/sets?error=catalog_not_synced", status_code=303)
    try:
        if pset.fb_set_id:
        ...
        return RedirectResponse(f"/dashboard/catalogs/{cat_id}/sets?error={str(e)[:80]}", status_code=303)
    return RedirectResponse(f"/dashboard/catalogs/{cat_id}/sets?synced=1", status_code=303)
